...
void leds_send() 
{
    ...
    // Wait until DMA transfer is complete, otherwise the latch delay below can
    // overlap active waveform output for longer frames.
    int dma_wait_us = 0;
    while (dma_active(DMA_CHAN) && dma_wait_us < 1000000) {
        usleep(1);
        dma_wait_us++;
    }
